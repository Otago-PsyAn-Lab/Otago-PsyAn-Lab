...
/**
 * A container that places a masking view on top of all other views. The masking
 * view can be faded in and out. Currently, the masking view is solid color
 * white.
 */
public class TransitionAnimationView extends FrameLayout {
    ...
    /**
     * Starts the transition of showing or hiding the mask. If showMask is true,
     * the mask will be set to be invisible then fade into hide the other views
     * in this container. If showMask is false, the mask will be set to be hide
     * other views initially. Then, the other views in this container will be
     * revealed.
     */
    public void startMaskTransition(boolean showMask) {
        ...
        mMaskingView.setVisibility(View.VISIBLE);
        if (showMask) {
            mAnimator = ObjectAnimator.ofFloat(mMaskingView, "alpha", 0.0f,
                    1.0f);
            mAnimator.start();
        } else {
            // asked to hide the view
            mAnimator = ObjectAnimator.ofFloat(mMaskingView, "alpha", 1.0f,
                    0.0f);
            mAnimator.start();
        }
    }
