 ...
 */
public class UserDetailContainerFragment extends Fragment {
    ...
    private UserExperimentsFragment mExperimentsFragment;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_user_detail_container,
                container, false);
    }
    ...
    public void onViewCreated(View view, Bundle savedState) {
        ...
        mFragmentCarousel = (FragmentCarouselView)view
        ...
        mExperimentsFragmentView = view
                .findViewById(R.id.right_fragment_container);
        ...
        mDetailFragment = (UserDetailFragment)fm
                .findFragmentByTag(UserDetailFragment.TAG);
        mExperimentsFragment = (UserExperimentsFragment)fm
                .findFragmentByTag(UserExperimentsFragment.TAG);

        if (mDetailFragment == null) {
            fragmentsAddedToFragmentManager = false;
            mDetailFragment = new UserDetailFragment();
            mExperimentsFragment = new UserExperimentsFragment();
        }
        ...
        switch (mLayoutMode) {
            case LayoutMode.VIEW_PAGER_AND_TAB_CAROUSEL:
                mDetailFragmentView = mLayoutInflater.inflate(
                        R.layout.user_detail_fragment_container, mViewPager,
                        false);
                mExperimentsFragmentView = mLayoutInflater.inflate(
                        R.layout.user_experiments_fragment_container,
                        mViewPager, false);
            ...
            case LayoutMode.FRAGMENT_CAROUSEL:
                ...
                mFragmentCarousel.setFragmentViews(
                        (TouchInterceptFrameLayout)mDetailFragmentView,
        ...
        // Display user if we have saved state.
        if (savedState != null) {
            showUser(false);
        }
    }
    ...
    private void addFragments(FragmentManager fm) {
        ...
        transaction.add(R.id.left_fragment_container, mDetailFragment,
                UserDetailFragment.TAG);
        transaction.add(R.id.right_fragment_container, mExperimentsFragment,
                UserExperimentsFragment.TAG);
    ...
    public void setUser(UserModel user) {
        boolean isDifferent = mUser == null || mUser.id != user.id ? true
                : false;
        mUser = user;
        showUser(isDifferent);
    }

    private void showUser(boolean isDifferentUser) {
        if (mUser == null) {
            return;
        }
        switch (mLayoutMode) {
            ...
            case LayoutMode.FRAGMENT_CAROUSEL:
                ...
                break;
            default:
                throw new IllegalStateException("Invalid LayoutMode "
                        + mLayoutMode);
        }
        ...
        mDetailFragment.setUser(mUser);
        mExperimentsFragment.setUser(mUser);
    }

    private void resetFragments() {
        mExperimentsFragment.resetAdapter();
    }
